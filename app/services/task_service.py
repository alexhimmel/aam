        ...
        for host in host_list:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    ssh_conn = pool.connect(host, port=ssh_port, timeout=10)
                    
                    if not ssh_conn:
                        raise Exception(f"无法连接到 {host}")
                    
                    # 执行命令
                    exec_result = ssh_conn.execute(command, timeout=timeout_seconds)
                    ssh_conn.close()
                    
                    if exec_result.get('success'):
                        host_output.append(f"=== {host} ===\n")
                        host_output.append(exec_result.get('output', ''))
